...
export const getLocalizedArtifact = (titleTag) => {
  ...
  const actions = {
    ...
    [LANGUAGE_SPANISH]: () =>
      artifactSpanish.filter((x) => x.tag === titleTag).pop(),
    [LANGUAGE_FRENCH]: () =>
      artifactFrench.filter((x) => x.tag === titleTag).pop(),
    [LANGUAGE_GERMAN]: () =>
      artifactGerman.filter((x) => x.tag === titleTag).pop(),
    [LANGUAGE_CHINESE]: () =>
      artifactChinese.filter((x) => x.tag === titleTag).pop(),
    [LANGUAGE_JAPANESE]: () =>
      artifactjapanese.filter((x) => x.tag === titleTag).pop(),
  };
  return (
    actions[settings.language]() ||
    (() =>
      console.log("Localization.js: " + titleTag + " not found for " + lang))
  );
};
...
export const getLocalizedStringConstant = (constantTag) => {
  ...
  return (
    actions[settings.language]() ||
    (() =>
      console.log("Localization.js: " + constantTag + " not found for " + lang))
  );
};
